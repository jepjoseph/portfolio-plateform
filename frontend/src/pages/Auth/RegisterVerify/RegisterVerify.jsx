...
function RegisterVerify() {
  ...
  const { verifyRegistration, operation, clearError } = useAuth();
  ...
  const isSubmitting = operation === AUTH_OPERATION.VERIFYING_REGISTRATION;
  ...
  useEffect(() => {
    ...
    document.title = "Verify Email | Portfolio Platform";
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!result?.continuationToken) {
        throw new Error(
          "The server did not return a password-setup continuation token.",
        );
      }
      ...
      navigate("/auth/set-password", {
        state: {
          ...
          email: result.email || maskedEmail,
  ...
  /*
   * The challenge exists in navigation state.
   * Direct access or page reload returns the
  ...
  return (
    <section
      className="register-verify-page"
      aria-labelledby="register-verify-title"
    >
      <header className="register-verify-header">
        <span className="register-verify-eyebrow">Verify your email</span>

        <h2 id="register-verify-title">Check your email</h2>
      ...
      <div className="register-verify-summary">
        <div>
          <span>Code expires in</span>

          <strong className={isExpired ? "register-verify-expired" : ""}>
            {expirationTime
      ...
      {isExpired ? (
        <div className="register-verify-alert" role="alert">
          <span aria-hidden="true">!</span>

          <div>
            <strong>This code has expired</strong>

            <p>Return to registration to request a new verification code.</p>
          </div>
        </div>
      ) : null}
      ...
      <form className="register-verify-form" onSubmit={handleSubmit} noValidate>
        <fieldset
          className="register-verify-code-fieldset"
        ...
        >
          ...
          <div
            className={[
              "register-verify-code-inputs",
              fieldError ? "register-verify-code-inputs--error" : "",
            ]
              .filter(Boolean)
              .join(" ")}
            onPaste={handlePaste}
          ...
          {fieldError ? (
            <span className="register-verify-field-error" role="alert">
              {fieldError}
            </span>
          ) : null}
        </fieldset>
        ...
        <button
          type="submit"
          className="register-verify-submit"
          disabled={isSubmitting || isExpired}
        >
          {isSubmitting ? (
            <>
              <span className="register-verify-spinner" aria-hidden="true" />
              Confirming code...
            </>
          ) : (
            <>
      ...
      <div className="register-verify-help">
        <p>Need a new code?</p>
